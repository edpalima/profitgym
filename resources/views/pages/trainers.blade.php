        ...
        <div class="container">
            ...
            <div class="row">
                @foreach ($trainers as $trainer)
                        ...
                        <div class="trainer-card">
                            ...
                            <div class="trainer-info">
                                ...
                                <div class="experience">
                                    {{ $trainer->years_of_experience ?? '7' }} years of experience teaching
                                    {{ $trainer->specialization }},
                                    focusing on mindfulness, flexibility, and mental well-being
                                </div>
                                <div class="actions">
                                    <button class="view-details-btn" data-toggle="modal"
                    ...
                    <div class="modal fade" id="trainerModal-{{ $trainer->id }}" tabindex="-1" role="dialog"
                        aria-labelledby="trainerModalLabel-{{ $trainer->id }}" aria-hidden="true">
                        <div class="modal-dialog modal-xl" role="document">
                            <div class="modal-content dark-modal">
                                <div class="modal-header orange-bg">
                                    <h5 class="modal-title" id="trainerModalLabel-{{ $trainer->id }}">
                                        {{ $trainer->first_name }} {{ $trainer->last_name }}</h5>
                                    ...
                                    <div class="row">
                                        <div class="col-md-5">
                                            <img src="{{ asset('storage/' . $trainer->image) }}"
                                                class="img-fluid mb-3 trainer-modal-img" alt="{{ $trainer->first_name }}">
                                            <div class="trainer-contact-info">
                                                <h5 class="orange-text">Contact Information</h5>
                                                <p><i class="fa fa-envelope orange-text"></i> <strong>Email:</strong>
                                                    {{ $trainer->email }}</p>
                                                <p><i class="fa fa-phone orange-text"></i> <strong>Phone:</strong>
                                                    {{ $trainer->phone }}</p>
                                                <p><i class="fa fa-clock-o orange-text"></i> <strong>Experience:</strong>
                                                    {{ $trainer->years_of_experience ?? '7' }} years</p>
                                            </div>
                                        </div>
                                        <div class="col-md-7">
                                            <h4 class="orange-text">About {{ $trainer->first_name }}</h4>
                                            <p class="trainer-bio">
                                                {{ $trainer->bio ?? 'Lorem ipsum dolor sit amet, consectetur adipiscing elit. Nullam in dui mauris. Vivamus hendrerit arcu sed erat molestie vehicula. Sed auctor neque eu tellus rhoncus ut eleifend nibh porttitor.' }}
                                            </p>
                                                ...
                                                <table class="table table-dark schedule-table">
                                                    ...
                                                    <tbody>
                                                        @foreach ($trainer->schedules as $schedule)
                                                            <tr>
                                                                <td>{{ $schedule->day }}</td>
                                                                <td>{{ date('g:i A', strtotime($schedule->start_time)) }} -
                    ...
                    <div class="modal fade" id="rateFeedbackModal-{{ $trainer->id }}" tabindex="-1" role="dialog"
                        aria-labelledby="rateFeedbackModalLabel-{{ $trainer->id }}" aria-hidden="true">
                        <div class="modal-dialog modal-xl" role="document">
                            <div class="modal-content dark-modal">
                                <div class="modal-header orange-bg">
                                    <h5 class="modal-title" id="rateFeedbackModalLabel-{{ $trainer->id }}">Rate &
                                        Feedback for {{ $trainer->first_name }}</h5>
                                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                        <span aria-hidden="true">&times;</span>
                                    </button>
                                </div>
                                ...
                                <form action="{{ Auth::check() ? route('trainers.rate', $trainer->id) : '#' }}"
                                    method="POST">
                                    @csrf
                                    <div class="modal-body">
                                        <div class="row">
                                            <div class="col-md-6">
                                                <div class="trainer-rate-card text-center">
                                                    <img src="{{ asset('storage/' . $trainer->image) }}"
                                                        class="img-fluid rounded-circle mb-3 rate-trainer-img"
                                            ...
                                            <div class="col-md-6">
                                                @guest
                                                    <div class="alert alert-warning">
                                                        <p>Please <a href="{{ route('login') }}">login</a> to submit your
                                                            feedback.</p>
                                                    </div>
                                                @endguest
                                                <div class="form-group">
                                                    ...
                                                    <div class="rating-stars text-center">
                                                        @for ($i = 5; $i >= 1; $i--)
                                                            <input type="radio"
                                                                id="star{{ $i }}-{{ $trainer->id }}"
                                                                name="rating" value="{{ $i }}"
                                                                {{ $i == 5 ? 'checked' : '' }}
                                                                {{ Auth::guest() ? 'disabled' : '' }}>
                                                            <label for="star{{ $i }}-{{ $trainer->id }}"><i
                                                ...
                                                <div class="form-group">
                                                    ...
                                                    <textarea class="form-control dark-input" id="feedback-{{ $trainer->id }}" name="feedback" rows="5"
                                                        placeholder="Share your experience with {{ $trainer->first_name }}..." {{ Auth::guest() ? 'disabled' : '' }}></textarea>
                                                </div>
                                                <div class="form-group">
                                                    ...
                                                    <div class="recommend-options">
                                                        <div class="form-check form-check-inline">
                                                            <input class="form-check-input" type="radio"
                                                                name="recommend" id="recommend-yes-{{ $trainer->id }}"
                                                                value="1" checked
                                                                {{ Auth::guest() ? 'disabled' : '' }}>
                                                            <label class="form-check-label"
                                                                for="recommend-yes-{{ $trainer->id }}">Yes</label>
                                                        </div>
                                                        <div class="form-check form-check-inline">
                                                            <input class="form-check-input" type="radio"
                                                                name="recommend" id="recommend-no-{{ $trainer->id }}"
                                                                value="0" {{ Auth::guest() ? 'disabled' : '' }}>
                                                            <label class="form-check-label"
                                                                for="recommend-no-{{ $trainer->id }}">No</label>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                    ...
                                    <div class="modal-footer dark-modal-footer">
                                        ...
                                        <button type="submit" class="btn orange-btn"
                                            {{ Auth::guest() ? 'disabled' : '' }}>Submit Feedback</button>
                                        @guest
